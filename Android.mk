...
DTS_FILES = $(wildcard ./arch/arm/boot/dts/$(DTS_NAME)*.dts)
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_OUT) $(KERNEL_CONFIG) $(KERNEL_HEADERS_INSTALL)
	$(MAKE)  O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=arm-eabi- -j4
	$(MAKE)  O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=arm-eabi- modules -j4
	$(MAKE)  O=$(KERNEL_OUT) INSTALL_MOD_PATH=$(KERNEL_MODULES_INSTALL) INSTALL_MOD_STRIP=1 ARCH=arm CROSS_COMPILE=arm-eabi- modules_install -j4
	$(mv-modules)
	$(clean-module-folder)
	$(append-dtb)

$(KERNEL_HEADERS_INSTALL): $(KERNEL_OUT) $(KERNEL_CONFIG)
	$(MAKE)  O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=arm-eabi- headers_install -j4

kerneltags: $(KERNEL_OUT) $(KERNEL_CONFIG)
	$(MAKE)  O=$(KERNEL_OUT) ARCH=arm CROSS_COMPILE=arm-eabi- tags -j4
...
bootimage: $(INSTALLED_DTIMAGE_TARGET)
	./mkbootimg  --kernel $(KERNEL_OUT)/arch/arm/boot/zImage --ramdisk ./ramdisk.img \
		...
		--base 0x00000000 --pagesize 2048 --ramdisk_offset 0x02000000 --tags_offset 0x00000100 --dt $(KERNEL_OUT)/dt.img  --output $(KERNEL_OUT)/boot.img
